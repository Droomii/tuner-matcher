...
<div class="card">
			...
			<h4 class="card-title">피아노 정보 수정</h4>
			</div>
						...
						<form data-toggle="validator" id="form" role="form" name="form" class="form" action="/piano/DoEditPiano.do" method="post" autocomplete="off" onsubmit="doSubmit();">
							...
							<div class="form-body">
								...
								<div class="col-xs-12 red" style="margin-bottom:1em">*표시된 항목은 필수 입력사항입니다.</div>
									<div class="col-md-12">
										<div class="form-group has-feedback">
											...
											<input value="<%=CmmUtil.nvl(pDTO.getPiano_name())%>" type="text" id="piano_name" maxlength="45" class="form-control" placeholder="피아노의 이름을 지어주세요" name="piano_name" required >
									...
									<div class="col-md-12">
										<div class="form-group has-feedback">
											...
											<input value="<%=CmmUtil.nvl(pDTO.getBrand())%>" type="text" id="brand" class="form-control" maxlength="20" placeholder="예) 삼익, 영창, 야마하" name="brand" required>
											<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
											<div class="help-block with-errors"></div>
										</div>
									</div>
									<div class="col-md-12">
										<div class="form-group has-feedback">
											...
											<input type="text" value="<%=CmmUtil.nvl(pDTO.getPlayer_type())%>" id="player_type" class="form-control" maxlength="20" placeholder="예)가정용, 전공자용, 학원용, 공연용 등" name="player_type" required>
											<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
											<div class="help-block with-errors"></div>
										</div>
									</div>
									<div class="col-md-12">
										<div class="form-group has-feedback">
											...
											<input type="text" value="<%=CmmUtil.nvl(pDTO.getPiano_desc())%>" id="piano_desc" class="form-control" placeholder="피아노의 정보를 간략히 적어주세요(상세위치, 모델 등)" name="piano_desc">
											<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
											<div class="help-block with-errors"></div>
										</div>
									</div>
								...
								<div class="row">
									<div class="col-md-12">
										<div class="form-group has-feedback">
											...
											<!-- /modal -->
											<input value="<%=CmmUtil.nvl(pDTO.getSerial())%>" type="text" id="serial" class="form-control" placeholder="피아노 스케일(덮개를 열면 보이는 철재 구조물)에 적인 일련번호를 적어주세요" name="serial">
											<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
											<div class="help-block with-errors"></div>
										</div>
									</div>
									<div class="col-lg-4 col-sm-6">
										<div class="form-group">
											...
											<input value="<%=CmmUtil.nvl(pDTO.getLast_tuned_date()).equals("") ? "" : pDTO.getLast_tuned_date().substring(0, 10)%>" type="date" id="last_tuned_date" class="form-control" name="last_tuned_date" data-toggle="tooltip" data-trigger="hover" data-placement="right" data-title="정확할 필요는 없습니다" data-original-title="" title="">
										</div>
									</div>
								</div>
								<div class="row">
										...
										<div class="form-group">
											...
											<div class="input-group">
												<label class="display-inline-block custom-control custom-radio ml-1">
													<input type="radio" name="piano_type" value="0" <%if(pDTO.getPiano_type().equals("0")){ %>checked="checked"<%} %> class="custom-control-input">
													<span class="custom-control-indicator"></span>
													<span class="custom-control-description ml-0">업라이트</span>
												</label>
												<label class="display-inline-block custom-control custom-radio">
													<input type="radio" name="piano_type" value="1" <%if(pDTO.getPiano_type().equals("1")){ %>checked="checked"<%} %> class="custom-control-input">
													<span class="custom-control-indicator"></span>
								...
								<div class="form-group has-feedback" data-toggle="tooltip" data-placement="top" data-original-title="피아노 위치에 따라 주변 조율사에게 노출됩니다" onclick="goPopup();">
									...
									<div class="input-group">
										<input value="<%=CmmUtil.nvl(pDTO.getAddr())%>" type="text" name="addr" id="addr" class="form-control" placeholder="주소를 검색해주세요" required  data-error="필수 입력사항입니다">
								...
								<input value="<%=CmmUtil.nvl(pDTO.getSgg_code())%>" type="text" name="sgg_code" id="sgg_code" hidden="hidden">
								<input value="<%=CmmUtil.nvl(pDTO.getSido_name())%>" type="text" name="sido_name" id="sido_name" hidden="hidden">
								<input value="<%=CmmUtil.nvl(pDTO.getSgg_name())%>" type="text" name="sgg_name" id="sgg_name" hidden="hidden">
								<input value="<%=CmmUtil.nvl(pDTO.getEmd_name())%>" type="text" name="emd_name" id="emd_name" hidden="hidden">
								<input value="<%=CmmUtil.nvl(pDTO.getLi_name())%>" type="text" name="li_name" id="li_name" hidden="hidden">
